...
## Feedback и подготовка к обучению

В Gradio добавлены кнопки **Полезно**, **Не полезно**, **Неверный источник** и **Не найден документ**. Каждая оценка сохраняется в таблицу `trace_feedback` вместе с `run_id`, поэтому feedback можно связать с route, источниками, latency и полным operational trace.

Feedback не выполняет автоматическое fine-tuning. Он создаёт материал для анализа ошибок:

```text
не найден документ → улучшить chunking/retrieval
неверный источник → улучшить citation validation
не полезно → проверить route, context и answer
полезно → добавить положительный regression case
```
